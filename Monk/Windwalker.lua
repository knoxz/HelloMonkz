...
local function OptimallyTargetedCast(SpellObject, DebugMessage)
    ...
    elseif
     ...
     then
        ...
        if BestUnit and BestUnit:GUID() == Target:GUID() and BestUnit:IsSpellInRange(SpellObject) then
            --   HR.Cast(SpellObject, nil, nil, not Target:IsInMeleeRange(8))
            return {["spell"] = SpellObject, ["multitarget"] = false}
        else
            if BestUnit:IsSpellInRange(SpellObject) then
...
local function UseCooldowns()
    ...
    if S.InvokeXuenTheWhiteTiger:IsReady() and Target:IsInRange(15) then
    ...
    if
        S.EnergizingElixir:IsReady() and
            ((Player:ChiDeficit() >= 2 and EnergyTimeToMaxRounded() > 2) or Player:ChiDeficit() >= 4) and
            Target:IsInRange(15)
     then
        return {["spell"] = S.EnergizingElixir, ["multitarget"] = false}
    end
    if
        S.TouchOfDeath:IsReady() and (Target:Health() < UnitHealthMax("player") or (Target:HealthPercentage() < 15)) and
            Target:IsSpellInRange(S.TouchOfDeath)
     then
        return {["spell"] = S.TouchOfDeath, ["multitarget"] = false}
    end
    if S.WeaponsOfOrder:IsReady() and Target:IsInRange(10) then
...
local function SpendChi(force_spend, chi_costs, chi_values)
    if
        S.SpinningCraneKick:IsReady() and ComboStrike(S.SpinningCraneKick) and chi_costs[S.SpinningCraneKick] == 0 and
            Target:IsInRange(8)
     then
        return OptimallyTargetedCast(S.SpinningCraneKick, "Chiji Proc")
    end
    if S.FistsOfFury:IsReady() and Target:IsSpellInRange(S.FistsOfFury) then
    ...
    if sck_efficiency > max(rsk_efficiency, rjw_efficiency, bok_efficiency) then
        if S.SpinningCraneKick:IsReady() and ComboStrike(S.SpinningCraneKick) and Target:IsInRange(8) then
...
local function SpendEnergy(force_spend)
    if S.FistOfTheWhiteTiger:IsReady() and Player:ChiDeficit() >= 3 and Target:IsSpellInRange(S.FistOfTheWhiteTiger) then
...
local function WeaponsOfOrderHandler()
    ...
    if
        S.SpinningCraneKick:IsReady() and Player:BuffUp(S.DanceOfChijiBuff) and ComboStrike(S.SpinningCraneKick) and
            Target:IsInRange(8)
     then
    ...
    if
        S.FistsOfFury:IsReady() and EnemiesCount8 >= 2 and Player:BuffRemains(S.WeaponsOfOrder) < 1 and
            Target:IsSpellInRange(S.FistsOfFury)
     then
    ...
    if S.SpinningCraneKick:IsReady() and ComboStrike(S.SpinningCraneKick) and EnemiesCount8 >= 3 and Target:IsInRange(8) then
    ...
    if S.WhirlingDragonPunch:IsReady() and Player:BuffUp(S.WhirlingDragonPunchBuff) and Target:IsInRange(8) then
    ...
    if S.FistsOfFury:IsReady() and Player:BuffUp(S.StormEarthAndFire) and Target:IsInRange(8) then
...
local function SpecialCases()
    ...
    -- Faeline stomp on CD
    if S.FaelineStomp:IsReady() and Target:IsInRange(8) then
